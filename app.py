...
async def menu_info(callback: types.CallbackQuery):
    text = (
        ...
        "2. Оплатите доступ через кнопку «💳 Оплата» с помощью Telegram Stars.\n"
        "3. Скопируйте присланный ключ.\n"
        "4. Вставьте ключ в приложение и подключитесь."
    )
    await callback.answer()
...
async def process_subscription_payment(callback: types.CallbackQuery):
    ...
    if user_id in pending_invoices:
        await callback.answer("У вас уже есть незавершенный платеж. Завершите или отмените его перед созданием нового.", show_alert=True)
        return
